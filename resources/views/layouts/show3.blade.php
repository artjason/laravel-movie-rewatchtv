...
@section('content')
    <div class="border-b border-gray-800 movie-info">
        <div class="container flex px-4 py-16 mx-auto">
            ...
            <div class="ml-4 lg:ml-24 sm:ml-6 md:ml-6">
                <h2 class="text-base font-semibold lg:text-2xl md:text-xl">{{ $actor['name'] }}
                </h2>
                 @if($actor['also_known_as'])
                <div class="py-1 text-sm text-yellow-500">{{ 'Also known as ' . $actor['also_known_as'][0] }}</div>
                @endif
                <div
                    class="flex mt-2 text-xs text-gray-400 lg:text-base lg:items-center md:items-center md:text-sm md:flex-row">
                    @include('icons.star')
                    <span class="ml-1">{{ round($actor['popularity'] * 0.1, 2) . '%' }}</span>
                ...
                <div class="mt-5 lg:mt-12 md:mt-7">
                    ...
                    <div class="container flex gap-5 px-4 py-4 lg:mt-4">
                        ...
                        @foreach ($actor['movie_credits']['cast'] as $pr)
                            @if ($pr['backdrop_path'] && $loop->index < 5)
                                <div>
                                    ...
                                    <div
                                        ...
                                        <a href="{{ route('movies.show', $pr['id']) }}">{{ $pr['title'] }} </a></div>
     ...
     @if(count($actor['movie_credits']) > 0)
    ...
    <div class="mx-auto mt-5 ">
        ...
        <ul class="container grid grid-cols-3 mx-auto">
            @foreach ($actor['movie_credits']['cast'] as $cast)
             <li class="py-1"><a href="{{route('movies.show', $cast['id'])}}" class="text-xs hover:text-blue-500">{{$cast['title']}}  </a></li>
            @endforeach
        </ul>
    </div>
   </div>
    ...
    <div class="container px-4 py-5 mx-auto">
        ...
        <div class="mx-auto mt-5 ">
            ...
            <ul class="container grid grid-cols-3 mx-auto">
                @foreach ($actor['tv_credits']['cast'] as $cast)
