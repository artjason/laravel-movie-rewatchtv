...
@section('content')
    <div class="border-b border-gray-800 movie-info">
        <div class="container flex px-4 py-16 mx-auto">
            ...
            <div class="ml-4 lg:ml-24 sm:ml-6 md:ml-6">
                        ...
                        @foreach ($movie['credits']['crew'] as $crew)
                            @if ($loop->index < 2)
                                <div class="mr-8">
                                    ...
                                    <div class="text-xs text-gray-400 md:text-sm lg:text-sm">{{ $crew['job'] }}</div>
                                </div>
                            @endif
                        @endforeach
                ...
                <div x-data="{ openMovie: false, toggle() { this.openMovie = !this.openMovie } }">
                    @if (count($movie['videos']['results']) > 0)
                        <div class="mt-6 lg:mt-14 md:mt-10">
                            <button @click="toggle()"
                                class="inline-flex items-center px-2 py-2 text-xs font-semibold text-gray-900 transition duration-150 ease-in-out bg-orange-500 rounded xl:px-4 xl:py-3 lg:px-5 lg:py-4 hover:bg-orange-400">
                                @include('icons.play') <span class="ml-2"> Play Trailer</span>
                            </button>
                        </div>
                    @endif

                    {{-- Movie Trailer Modal --}}
                    <div x-show="openMovie" x-transition:enter="transition ease-out transform"
                        ...
                        <div class="container mx-auto overflow-y-auto rounded-lg lg:px-32">
                            <div class="rounded bg-zinc-800">
                                <div class="flex justify-end pt-2 pr-4">
                                    <button @click="toggle()" @keydown.escape.window="openMovie = false"
    ...
    <div class="border-b border-gray-800 movie-cast">
        ...
        <div class="container px-5 pt-1 mx-auto mb-24">
            <div
                class="grid grid-cols-2 gap-2 sm:gap-4 md:gap-4 lg:gap-6 xl:gap-8 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
                @include('layouts.moviecast')
            </div>
        </div>
    </div>
    <div class="mb-auto border-b border-gray-800">
        <div class="container px-4 py-5 mx-auto">
            <h2 class="mb-10 text-base font-semibold sm:text-2xl md:text-3xl lg:text-4xl xl:text-4xl">Scenes</h2>
        </div>
        <div class="container px-5 pt-1 mx-auto mb-20">
            @if (count($movie['images']['backdrops']))
                <div
                    class="grid grid-cols-2 gap-2 sm:gap-4 md:gap-4 lg:gap-6 xl:gap-8 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3">
                    @include('layouts.scenes')
                </div>
            @else
    ...
    <div class="mb-auto border-b border-gray-800">
        ...
        <div class="container px-4 py-5 mx-auto mt-5 mb-24">
            @if (count($movie['reviews']['results']) > 0)
                @foreach ($movie['reviews']['results'] as $review)
                <div class="mt-5 mb-5">
                    <div class="px-4 text-xs md:text-sm lg:text-sm xl:text-1xl"> {{ $review['author'] }} <span class="float-right text-xs md:text-sm lg:text-sm text-slate-400">
                            {{ $review['created_at'] }} </span></div>
                    <span class="px-4 text-xs text-slate-400 md:text-sm lg:text-sm"> {{ $review['author_details']['username'] }}</span>
                    <div class="flex px-4 py-2">
                        <span class="py-1"> @include('icons.star') </span>
                        <span class="ml-1 text-xs md:text-sm lg:text-sm">{{ $review['author_details']['rating'] . '/10' }} </span>
                    </div>
                    <div class="px-4 py-4 mt-1 mb-8 text-xs italic text-justify text-blue-500 lg:mt-5 lg:text-1xl md:mt-4 md:text-sm"> {{ $review['content'] }} </div>
                </div>
                    @endforeach
            @else
                <div class="text-xs text-yellow-500 md:text-sm lg:text-base">Nothing to show </div>
